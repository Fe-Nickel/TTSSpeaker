...
class IndexTTS2:
    ...
    # 原始推理模式
    # 将 spk_audio_prompt 设为可选，新增 spk_dict 参数
    def infer(self, spk_audio_prompt=None, text="", output_path=None,
              spk_dict=None,  # <--- 新增参数
        ...
        if stream_return:
            return self.infer_generator(
                spk_audio_prompt, text, output_path, spk_dict, # <--- 透传 spk_dict
                emo_audio_prompt, emo_alpha, emo_vector, use_emo_text, emo_text, 
                use_random, interval_silence, verbose, max_text_tokens_per_segment, 
                stream_return, more_segment_before, **generation_kwargs
            )
        else:
            try:
    ...
    def infer_generator(self, spk_audio_prompt=None, text="", output_path=None,spk_dict=None,
              emo_audio_prompt=None, emo_alpha=1.0,
              emo_vector=None,
              use_emo_text=False, emo_text=None, use_random=False, interval_silence=200,
        ...
        if emo_audio_prompt is None:
            if spk_audio_prompt is not None:
                ...
                # 使用了 spk_dict，没有 spk_audio_prompt，情绪特征直接复用音色特征
                emo_audio_prompt = "__loaded_speaker_dict__"
                emo_alpha = 1.0

        # 如果参考音频改变了，才需要重新生成, 提升速度
        # ================= 修改音色缓存逻辑 =================
        # 如果传入了 spk_dict，直接使用它；否则走原有的音频提取逻辑
        if spk_dict is not None:
            # 接口3/4：直接使用传入的音色字典
            self.cache_spk_cond = spk_dict["spk_cond_emb"].to(self.device)
            self.cache_s2mel_style = spk_dict["style"].to(self.device)
            self.cache_s2mel_prompt = spk_dict["prompt_condition"].to(self.device)
            self.cache_mel = spk_dict["ref_mel"].to(self.device)
            # 设置一个占位符，防止后续逻辑因为 spk_audio_prompt 为 None 而报错
            self.cache_spk_audio_prompt = "__loaded_speaker_dict__" 
        ...
        if emo_audio_prompt == "__loaded_speaker_dict__":
            # 直接复用 spk_cond_emb 作为 emo_cond_emb，跳过音频加载
    ...
    def extract_speaker_condition(self, audio_path, verbose=False):
        print(f">> Extracting speaker condition from: {audio_path}")
    ...
    def save_speaker_condition(self, spk_dict, save_path):
        # 移到CPU保存，避免跨设备加载报错，并减小显存占用
        cpu_dict = {k: v.cpu() for k, v in spk_dict.items()}
        dir_name = os.path.dirname(save_path)
        if dir_name:
            os.makedirs(dir_name, exist_ok=True)
        torch.save(cpu_dict, save_path)
        print(f">> Speaker condition saved to: {save_path}")

    def load_speaker_condition(self, load_path):
        spk_dict = torch.load(load_path, map_location=self.device)
        print(f">> Speaker condition loaded from: {load_path}")
        return spk_dict
